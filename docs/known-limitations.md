...
## RESOLVED: technical values (paths, hashes, IDs, enums) visually reordered under Arabic/RTL
...
values like `local_unverified_source`, model/profile/candidate IDs,
backend names, quantization codes) rendered in a plain
`.kv-row-value.mono` span with no explicit `dir="ltr"`, so the browser's
bidi algorithm reorders weak-direction characters (backslashes)
according to the surrounding Arabic paragraph direction. Pre-existing
and app-wide, not introduced by Phase A.

Fixed with a shared `components/TechnicalValue.tsx` (`dir="ltr"` + CSS
`unicode-bidi: isolate` - presentation-only, adds no characters to the
DOM text, so copy/paste always yields the exact original string),
applied across every page that renders a technical value:
`AdvancedConsole.tsx`, `Run.tsx`, `Models.tsx`, `Profiles.tsx`,
`Settings.tsx`, `Discover.tsx`, `Hardware.tsx`, `Health.tsx`, and
`Optimize.tsx` (both the model-recommendation result and the tuning
run's candidate IDs/backend statuses/saved-profile ID). Native
...
label, a duplicate-count sentence, tuning's "was not measured" reasons)
are deliberately left unwrapped so Arabic phrasing keeps its normal
reading order.
...
`TechnicalValue.test.tsx` plus a dedicated case in every affected page's
own test file (`Hardware.test.tsx`, `Health.test.tsx`, and
`Optimize.test.tsx` were new - none of the three had any test coverage
before this pass). This closes the issue app-wide; no page with a
`kv-row-value`/`mono`/`path-text` technical-value class remains unfixed.
